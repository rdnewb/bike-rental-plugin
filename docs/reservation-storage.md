...
As of 0.5.1, Active reservations use their normal occupied interval until the locked database UTC time is strictly greater than `occupied_end_utc`; only then does the effective end become indefinite. The stored endpoints are never extended. Query selection, sweep events, and proposed allocation checks share that rule. Overdue status is evaluated on every locked read/write without relying on cron. Existing future commitments are preserved even if a subsequent overdue return causes overcapacity.

All Active saves require `start_utc <= database UTC now`, including creation, helper transitions, no-op retries, and full administrative edits. This uses rental start, not preparation start. Completed saves require an ended schedule unless recording actual return from an Active rental; such returns may be early. Previously completed early returns can retain their schedule on edits/retries. Future-start Completed records and unfinished non-Active completion are rejected. Staff selecting Completed for an Active rental confirms return; no independent return evidence platform is added. Initial creation and corrections remain limited to the six statuses, subject to these timing checks.

Actual active → completed saves atomically create a dated block for the old row's quantity and captured turnaround minutes, starting at database UTC now; zero turnaround creates no block. Completed rows themselves do not consume. These blocks are inspectable/editable in Fleet under the same rules as other blocks. Pre-upgrade future Active rows are retained with bounded claims; staff can correct them to Confirmed. No schema migration or automatic status rewrite is performed.
