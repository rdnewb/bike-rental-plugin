...
**Current version: Bike Rental Plugin 0.9.0 (rental schema 2). The licensing server is maintained separately as NT License Controller 0.2.0. Compatibility mode leaves booking enforcement OFF until explicitly enabled. Live HTTPS licensing deployment, WPForms signing, invitation delivery and Square sandbox acceptance remain pending.**

This repository contains only the rental product and its licensing client. The authoritative server, Products administration, server tests and API documentation now live in [rdnewb/nt-license-controller](https://github.com/rdnewb/nt-license-controller). Start with [client setup](docs/license-client-setup.md), [client architecture](docs/licensing-architecture.md) and [extraction verification](docs/licensing-phase1-verification.md). The administrator-only **License** tab retains activation, deactivation, manual/daily checks, encrypted key storage and seven-day bounded grace. The runtime remains byte-identical to 0.9.0, so no client version bump is needed. Build the client-only release with `python tools/package.py`. No Stripe billing, renewals or private update delivery is implemented.
