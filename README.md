...
**Current version: Bike Rental Plugin 0.9.2 (rental schema 2). The licensing server is maintained separately as NT License Controller 0.2.0. New-booking license enforcement defaults ON; only the BRP_LICENSE_ENFORCE wp-config.php constant can override it. Existing records and management stay accessible. License installation/activation has been reported working on the test site; WPForms signing, invitation delivery and Square sandbox acceptance remain pending.**

This repository contains only the rental product and its licensing client. The authoritative server, Products administration, server tests and API documentation now live in [rdnewb/nt-license-controller](https://github.com/rdnewb/nt-license-controller). Start with [client setup](docs/license-client-setup.md), [client architecture](docs/licensing-architecture.md) and [extraction verification](docs/licensing-phase1-verification.md). The administrator-only **License** tab retains activation, deactivation, manual/daily checks, encrypted key storage and seven-day bounded grace. Version 0.9.2 hides the controller URL and transmission description from the License page and clearly displays License Active with a disabled masked key field; Check License Now and Deactivate use the stored key. Original-key recovery is available in a collapsed section. Build the client-only release with `python tools/package.py`. No Stripe billing, renewals or private update delivery is implemented.
