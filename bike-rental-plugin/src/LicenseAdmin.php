...
final class LicenseAdmin {
 ...
 public static function render() {
  ...
  $s = License::state(); $notice = get_transient( 'brp_license_notice_' . get_current_user_id() );
  ...
  echo '<h2>License</h2><p>' . esc_html( License::enforced() ? 'Production enforcement is enabled for new reservations.' : 'Compatibility mode: licensing enforcement is disabled. Current booking operations continue.' ) . '</p><table class="widefat striped"><tbody>';
  ...
  echo '<p><label for="brp-license-key">License Key</label><br><input id="brp-license-key" type="password" autocomplete="new-password" class="large-text" name="license_key" value="" maxlength="100"></p><p>Leave blank to use the stored key. Activate contacts the controller; entering a key alone does not activate it. Deactivate before replacing a key. Keys are encrypted locally and never redisplayed.</p>';
  foreach ( array( 'activate' => 'Activate License', 'validate' => 'Check License Now', 'deactivate' => 'Deactivate License' ) as $key => $label ) { echo '<button class="button" name="operation" value="' . esc_attr( $key ) . '">' . esc_html( $label ) . '</button> '; }
