...
final class WPFormsWaiverProvider implements WaiverProvider {
	...
	public function settings( array $config ) {
		...
		} echo '</table>'; $ready = $this->configuration( $config ); echo '<p>' . esc_html( is_wp_error( $ready ) ? 'Misconfigured: ' . $ready->get_error_message() : 'Available and configured.' ) . '</p>';
	}
	...
	public function render( array $context, string $token ) {
		...
		try { return do_shortcode( '[wpforms id="' . (int) $config['form_id'] . '" title="false" description="false"]' ); }
	...
	public static function properties( $properties, $field, $form ) {
		...
		if ( $key && array_key_exists( $key, $expected ) && isset( $properties['inputs']['primary']['attr'] ) ) { $properties['inputs']['primary']['attr']['value'] = $expected[ $key ]; }
		return $properties;
	}
	...
	public static function process( $fields, $entry, $form ) {
		...
		$token = $entry['brp_waiver_token'] ?? ''; $ctx = Waivers::context( $token ); $configured = (int) ( WaiverSettings::get()['wpforms']['form_id'] ?? 0 );
	...
	public function verify_completion( array $context, string $submission ) {
		...
		$entry = wpforms()->entry->get( (int) $submission );
